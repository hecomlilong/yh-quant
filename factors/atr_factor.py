...
from utils.stock_util import get_codes, get_daily_conn_name
...
def compute_atr(ts_code='', begin_date='', end_date='', day_count=14):
    """
    ...
    for code in codes:
        try:
            ...
            for i in df_daily.index:
                atr.append(0.0)
                if i >= n:
                    atr[i] = df_daily.iloc[i - n:i+1]['tr_%s' % day_count].sum() / day_count

            df_daily['atr_%s' % day_count] = atr
            # 抛掉不用的数据
            df_daily.drop(['current_span', 'close_low', 'close_high'], 1, inplace=True)

            # 将信号保存到数据库
            ts_base = TuShareBase()
            ts_base.save_data(df=df_daily, collection=DB_CONN[name],
...
if __name__ == '__main__':
    compute_atr(ts_code='000001.SZ', begin_date='20190101', end_date='20190308')
